...
/**
 * Date control on a native `<input type="date">`.
...
export class AuInputDate {
  ...
  constructor() {
    ...
    afterRenderEffect(() => {
      ...
      applyNativeTemporalMinMax(this.host.nativeElement, this.minDate(), this.maxDate());
  ...
  private ensurePickerChrome(): void {
    ...
    if (!this.anchorHost?.isConnected) {
      ...
      this.renderer.addClass(parent, 'au-input-date__anchor');
    }
    ...
    if (!this.pickerIconEl?.isConnected) {
      ...
      this.iconRef.setInput('name', 'calendar');
