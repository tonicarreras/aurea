...
export const OVERVIEWS_ES: Record<string, ComponentDocOverview> = {
  ...
  textarea: {
    ...
    whenNotToUse: {
      title: 'Alternativas',
      items: ['Una sola línea → `input[auInputText]`.', 'Editor rico → componente de terceros.'],
    },
    anatomy: [
  ...
  autocomplete: {
    ...
    whenNotToUse: {
      title: 'Alternativas',
      items: ['Lista corta fija → `au-select`.', 'Texto libre sin lista → `input[auInputText]`.'],
    },
    anatomy: [
  ...
  accordion: {
    ...
    whenNotToUse: {
      ...
      items: [
        ...
        'Wizard secuencial numerado → `au-stepper`.',
      ],
    },
    anatomy: [
      {
  ...
  'input-password': {
    ...
    whenNotToUse: {
      title: 'Alternativas',
      items: ['OTP o PIN de longitud fija → patrón de input especializado.'],
    },
    anatomy: [
  ...
  'tag-input': {
    ...
    anatomy: [
      { region: 'Chips', detail: 'Etiquetas removibles con botón de quitar por chip.' },
  ...
  stepper: {
    intro: [
      'Wizard secuencial numerado: `au-stepper` + `button[auStep]` + `[auStepPanel]`.',
      'Usa `layout` horizontal/vertical y `linear` para bloquear saltos. Para nav de secciones usa `au-tabs`.',
    ],
    whenToUse: {
      title: 'Cuándo usarlo',
      items: [
        'Wizards multi-paso con estado de completado (`auStepCompleted`).',
        'Pasos opcionales o con error (`auStepOptional`, `auStepError`).',
      ],
    },
    whenNotToUse: {
      title: 'Alternativas',
      items: [
        'Nav de secciones / página sin números → `au-tabs`.',
    ...
    anatomy: [
      {
        ...
        detail: 'Triggers numerados con CSS `counter(stepper-index)`.',
      },
      {
        region: 'Paneles',
        detail: 'Una región activa a la vez.',
      },
    ],
    accessibility: [
      '`aria-current="step"` en el trigger activo.',
      '`aria-disabled` cuando el modo linear bloquea un paso futuro.',
    ],
  ...
  'description-list': {
    ...
    anatomy: [
      { region: 'Shell `dl`', detail: 'Rejilla CSS según `layout` y `columns`.' },
      {
        region: '`au-description-item`',
        detail: 'Renderiza `<dt>` y `<dd>`; host con `display: contents`.',
      },
      { region: '`dt` / `dd` nativos', detail: 'Proyección opcional para descripciones ricas.' },
    ],
    accessibility: [
