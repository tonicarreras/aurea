...
export interface ComponentDocOverview {
  /** Párrafos introductorios (HTML permitido: solo <code> en strings). */
  intro: string[];
...
const OVERVIEWS: Record<string, ComponentDocOverview> = {
  button: {
    intro: [
...
export function resolveComponentOverview(doc: ComponentDoc): ComponentDocOverview | null {
  return OVERVIEWS[doc.slug] ?? null;
}
